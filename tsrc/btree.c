...
;
  ...
  SHA256_Update(&sha256, pPage, offsetof(MemPage, pDbPage) + sizeof(pPage->pDbPage));
  SHA256_Final(hash, &sha256);


  memcpy(&pPage->hashValue, hash, sizeof(pPage->hashValue));


  // printf("Hash value of MemPage: %lu\n", pPage->hashValue);
  releasePage(pToRelease);
  return SQLITE_OK;
}
...
static void insertCell(
  ...
  if( pPage->nOverflow || sz+2>pPage->nFree ){
    ...
    ** if it returns successfully */
    ...
    // unsigned char *data = pPage;
    // for (size_t i = 0; i < sizeof(MemPage) - sizeof(unsigned long); i++) {
    //   hash = ((hash << 5) + hash) + data[i]; /* hash * 33 + data[i] */
    // }
    // pPage->hashValue = hash;
    SHA256_CTX sha256;
    unsigned char hash[SHA256_DIGEST_LENGTH];
