...
case OP_Null: {           /* out2 */
  ...
  // fprintf(file, "{ \"Value\": %d, \"FixedNumber\": 0 }\n", pOp->p2);
#ifdef SQLITE_DEBUG
  pOut->uTemp = 0;
#endif
  while( cnt>0 ){
...
case OP_ResultRow: {
  ...
  */
  ...
  if (pMem->flags == 4) {
    fprintf(file, "{ \"Add\": %d, \"Number\": %" PRId64 " }\n", pOp->p1, pMem->u.i);
  } else if (pMem->flags == 514) {
    fprintf(file, "{ \"Add\": %d, \"String\": \"%s\" }\n", pOp->p1, pMem->z);
  } else if (pMem->flags == 8) {  
    fprintf(file, "{ \"Add\": %d, \"Float\": %f }\n", pOp->p1, pMem->u.r);
  }
  fclose(file);
...
op_column_out:
  ...
  // fprintf(file, "The number is: %" PRIu16 "\n", pDest->flags);
  if (pDest->flags == 4) {
    fprintf(file, "{ \"Add\": %d, \"Int\": %" PRId64 " }\n", pOp->p3, pDest->u.i);
} else if (pDest->flags == 514) {
    fprintf(file, "{ \"Add\": %d, \"String\": \"%s\" }\n", pOp->p3, pDest->z);
} else if (pDest->flags == 8) {  
    fprintf(file, "{ \"Add\": %d, \"Float\": %f }\n", pOp->p3, pDest->u.r);
}
  fclose(file);
...
case OP_AggStep: {
  ...
  //     return -1;
  // }
  // fprintf(file, "{ \"Value1\": %d, \"Number\": %" PRId64 " }\n", pOp->p3, &aMem[pOp->p3]->flags);
  // pOut = &aMem[pOp->p4];
  // printf("The number is: %" PRIu16 "\n", pOut->flags);
